...
// import { Category, Post, PostStatus } from "@/lib/generated/prisma/client";
...
// components/post-card.tsx ✅ PUBLIC VERSION
"use client";

// ✅ Import PublicPost — do NOT use inline interface
import { PublicPost } from "@/components/types/public-post"; // ← YOU MUST CREATE THIS FILE
import { stripHtml } from "@/lib/utils";
import { format } from "date-fns";
import Image from "next/image";
import Link from "next/link";
...
export default function PostCard({ post }: PostProps) {
  const excerpt = stripHtml(post.content);
  const authorName = post.author || post.user?.name || "Anonymous";
    ...
    >
      <Card className="w-full border shadow-sm rounded-xl overflow-hidden p-0 gap-0">
        ...
        <CardContent className="p-4 pt-0 pb-4">
          ...
          <span className="text-sm text-muted-foreground font-medium">
            {format(post.createdAt, "dd/MM/yyyy")} by {authorName}
          </span>
        </CardContent>
      </Card>
    </Link>
