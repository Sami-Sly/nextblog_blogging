...
function ImagePlaceholderComponent(props: NodeViewProps) {
  ...
  // ✅ UploadThing hook — must be inside component
  ...
  if (!startUpload || !editor) return;

  try {
    const results = await startUpload(acceptedFiles);
    if (results?.length) {
      results.forEach((file) => {
        editor.chain().focus().setImage({ src: file.url.trim() }).run(); // ✅ .trim()
      });
    }
  } catch (error) {
...
//   };
  ...
  e.preventDefault();
  const valid = isValidUrl(url.trim()); // ✅ trim before validation
  if (!valid) {
    setUrlError(true);
    return;
  }
  if (url) {
    editor.chain().focus().setImage({ src: url.trim() }).run(); // ✅ .trim()
    extension.options.onEmbed(url.trim(), editor);
    setUrl("");
    setUrlError(false);
  }
};
